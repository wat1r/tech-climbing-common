...
/**
 * sql.SqlCommentParser is used to parse the comment position in sql
 * <p>
 * sql.SqlCommentParser thinks that a sql string is made up of three types of strings.
 ...
 * <p>
 * note:
 * (1)sql.SqlCommentParser cannot identify whether the string conforms to the SQL specification,
...
public class SqlCommentParser {
    ...
    public static void main(String[] args) {
        String content ="select employee_id, max(cast(organization_id as integer)) \n" +
                "organization_id \n" +
                "from src_ht_hr_ims.ims_rt_employee_organization where state=1\n" +
                "and jobtitle <>'--' --and employee_id='50019940'\n" +
                "group by employee_id   ";

        SqlCommentParser sqlCommentParser = new SqlCommentParser(content);
        String result = sqlCommentParser.removeCommentSql();
